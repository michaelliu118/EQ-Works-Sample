...
class DataVisualization:
    ...
    def daily_data_plot(self, source):
        df = self.matrix[source.__name__].set_index('date')
        # Construct the plot
        fig,axe = plt.subplots(1, len(df.columns),squeeze=False)
        y = 0
    ...
    def hour_data_plot(self,source):  # This method serves to plot all the hour-based data, and plot them in heatmap
        ...
        for column in columns:
            dataframe_dict[column] = pd.pivot_table(df[['hour', column]], index=['date'], columns=['hour'])
        ...
        x = 0
        fig, axe = plt.subplots(len(columns), 1, squeeze=False,figsize=(17, 17), constrained_layout=True)
        # fig.tight_layout()
